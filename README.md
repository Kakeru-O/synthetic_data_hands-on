...
`main.py` は完全にインタラクティブな marimo notebook です。ユーザーはセルを上から順に実行することでシミュレーションを完了させます。
...
* **QAレポート:**
    * MostlyAIから出力された総合的な「Similarity & Privacy」のメトリクス（Cosine Sim, Identical Match Rateなど）を確認します。
    * Data QA レポート（HTML）を開き、詳細な評価結果を閲覧できることを示します。
* **可視化:**
    * **Altair** を使用して1Dの分布（数値データはKDE密度プロット、カテゴリデータはDodge棒グラフ）を描画します。
    * 選択したカラムについて、元データ（青）と合成データ（赤）の分布を横並びまたは重ねて表示し、整合性を視覚的に示します。
